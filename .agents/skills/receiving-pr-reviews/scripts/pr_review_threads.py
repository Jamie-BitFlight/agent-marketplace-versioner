...
def reply_and_resolve_batch(
    ...
    for entry in entries:
        ...
        try:
            ...
            # `replied` reflects whatever succeeded before the failure -- a reply that already
            # posted is not undone by a subsequent resolve failure, so the caller must see it
            # rather than get no line at all and risk re-posting the same reply on retry.
            typer.echo(
