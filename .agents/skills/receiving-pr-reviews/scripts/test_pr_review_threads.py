...
# --- reply-and-resolve-batch: a mid-batch gh failure must not vanish silently ---------------------


def test_reply_and_resolve_batch_reports_partial_state_on_mid_batch_failure(
    mocker: MockerFixture, tmp_path: Path
) -> None:
    """A `gh` failure after a reply posts but before its resolve must surface, not vanish.

    Regression coverage: `reply_and_resolve_batch` previously wrapped no exception handling
    around its per-entry `gh` calls, so a transient resolve failure aborted the whole batch with
    an unhandled traceback and printed nothing for the in-flight entry -- leaving a caller unable
    to tell the reply had already posted, and at risk of re-posting the same reply on retry.
    """
    entries = [
    ...
    mocker.patch.object(
        ...
        "run_gh",
        side_effect=[
            '{"id": 1}',  # T1 reply succeeds
            "{}",  # T1 resolve succeeds
            '{"id": 2}',  # T2 reply succeeds
            subprocess.CalledProcessError(1, ["gh"]),  # T2 resolve fails
        ],
    )

    result = runner.invoke(app, ["reply-and-resolve-batch", "--pr", "3208", "--input-file", str(input_file)])

    assert result.exit_code == 1
    lines = [json.loads(line) for line in result.output.splitlines()]
    assert lines[0] == {"thread_id": "T1", "replied": True, "resolved": True}
    assert lines[1]["thread_id"] == "T2"
    assert lines[1]["replied"] is True
